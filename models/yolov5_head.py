...
class Yolov5_Head(nn.Module):
    ...
    def forward(self, inputs, targets=None):
        ...
        out_stride_8, loss1  = self.end_layer1(temp_8, targets, self.img_size, True)
        out_stride_16, loss2 = self.end_layer2(temp_16, targets, self.img_size, True)
        out_stride_32, loss3 = self.end_layer3(temp_32, targets, self.img_size, True)
        total_loss = loss1 + loss2 + loss3
        if self.training:
            return total_loss, [out_stride_8, out_stride_16, out_stride_32]
        else:
            return [out_stride_8, out_stride_16, out_stride_32]
